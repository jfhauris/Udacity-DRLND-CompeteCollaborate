...
def training_loop_scores(env, brain_name, agent, config):
    scores = []
    avg_scores = []
    ...
    for e in range(1, config.training.episode_count):
        ...
        for t in range(config.training.max_t):
            ...
            if any(done):
                break
            #print('train: state, next_State, reward, done, action \n')
            #print(state)
            #print(next_state)
            #print(reward)
            #print(done)
        ...
        if e > 100 and avg_scores[-1] > last_max and ((avg_scores[-1] - last_max) > 0.05 or avg_scores[-1] > config.training.solve_score):
            for i, to_save in enumerate(agent.agents):
                ...
                #torch.save(to_save.actor_local.state_dict(), os.getcwd() + f"score_{avg_scores[-1]:.2f}_actor_{i}.weights")
                #torch.save(to_save.critic_local.state_dict(), os.getcwd() + f"score_{avg_scores[-1]:.2f}_critic_{i}.weights")
